...
public class Physics
{
	...
	public Rocket MoveRocket(Rocket rocket, RocketForce force, Turn turn, Vector spaceSize, double dt)
	{
		...
		if (location.X < 0)
		{
			velocity = velocity with { X = Math.Max(0, velocity.X) };
		}

		if (location.X > spaceSize.X)
		{
			velocity = velocity with { X = Math.Min(0, velocity.X) };
		}

		if (location.Y < 0)
		{
			velocity = velocity with { Y = Math.Max(0, velocity.Y) };
		}

		if (location.Y > spaceSize.Y)
		{
			velocity = velocity with { Y = Math.Min(0, velocity.Y) };
		}
