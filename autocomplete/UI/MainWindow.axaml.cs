...
public partial class MainWindow : Window
{
	...
	{
		InitializeComponent();
		var a = new AutocompleteTask();
		//list
		FoundedItems = ["start typing"];
		AutocompleteList.ItemsSource = FoundedItems;
		//phrases
	...
	private (TimeSpan Elapsed, string[] FoundItems, int Count) FindItems(string prefix)
	{
		...
		foundItems = oneItem != null ? [oneItem] : [];
	...
	private void UpdateUi(Task<(TimeSpan, string[], int)> prevTask)
	{
		...
		var tuple = prevTask.IsFaulted
			? (TimeSpan.FromSeconds(2), ["... search timeout :("], -1)
			: prevTask.Result;
